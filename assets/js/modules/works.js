...
(function (X) {
  ...
  const works = {
    ...
    render() {
      ...
      return `
        <section class="app-view">
          ...
          <div class="grid auto" id="wk_grid"></div>
        </section>`;
    },

    afterRender() {
      X.utils.$$('#wk_cats .cat-pill').forEach(b => b.addEventListener('click', () => { this.cat = b.dataset.cat; this.renderGrid(); X.router.updateNav(); X.utils.$$('#wk_cats .cat-pill').forEach(x => x.classList.toggle('active', x.dataset.cat === this.cat)); }));
      X.utils.$('#wk_q').addEventListener('input', X.utils.debounce(e => { this.q = e.target.value; this.renderGrid(); }, 200));
    ...
    renderGrid() {
      ...
      if (!grid) return;
      const all = X.store.getWorks().filter(w => w.status === 'approved');
    ...
    openUpload() {
      ...
      X.utils.$('#up_file', inst.modal).addEventListener('change', async e => {
        ...
        if (!f) return;
        if (f.size > 256 * 1024) { X.ui.toast('file ≤256KB', 'err'); e.target.value = ''; return; }
      ...
      submit.addEventListener('click', async () => {
        ...
        const content = await X.utils.readText(file);
        const ext = file.name.split('.').pop().toLowerCase();
        const cur = X.auth.currentUser();
        X.store.createWork({ authorId: cur.id, name, desc, category, price, fileName: file.name, fileContent: content, fileType: ext });
    ...
    openDetail(workId) {
      ...
      if (!w) return;
      const author = X.store.getUser(w.authorId) || {};
      const cur = X.auth.currentUser();
      const ext = (w.fileName || '').split('.').pop().toLowerCase();
      ...
      if (canPreview) {
        ...
        if (ext === 'html') {
          ...
          iframe.srcdoc = w.fileContent;
        } else {
          const pre = X.utils.h('pre', { class: 'code' });
          pre.textContent = w.fileContent || '';
      ...
      const av = author.avatarType === 'dataurl'
        ? `<img class="avatar md clickable" src="${author.avatar}" />`
        : `<div class="avatar md" style="display:grid;place-items:center;font-size:22px">${author.avatar || '👤'}</div>`;
      right.innerHTML = `
        <div class="row" style="align-items:center;margin-bottom:12px">
          ${av}
          <div><div style="font-weight:600">${X.utils.escape(author.username || '?')}</div><div class="dim" style="font-size:12px">${X.utils.relTime(w.createdAt)}</div></div>
        </div>`;
      const actions = X.utils.h('div', { class: 'col' });
      // 点赞
      const liked = cur && w.likedBy.includes(cur.id);
      actions.appendChild(X.utils.h('button', { class: 'btn ' + (liked ? 'primary' : 'ghost'), onclick: () => { this.doLike(workId); } }, [`${liked ? '❤️' : '🤍'} ${t('works.like')} (${w.likes})`]));
      // 下载
    ...
    doLike(workId) {
      if (!X.auth.requireLogin()) return;
      const cur = X.auth.currentUser();
      const r = X.store.toggleLike(workId, cur.id);
      if (!r) return;
      X.ui.toast(X.t('ok.liked'), 'ok', 1200);
      // 刷新详情
      const cur2 = X.utils.$('.modal.show');
      if (cur2) { cur2.remove(); this.openDetail(workId); }
    },
    ...
    async doDownload(workId) {
      ...
      if (!w) return;
      // 游戏分区需实名
      if (w.category === 'game' && !cur.realname) {
      ...
      // 自己的作品直接下载
      if (w.authorId === cur.id) {
        X.utils.downloadText(w.fileName, w.fileContent, 'text/plain');
        X.ui.toast(t_ok(), 'ok'); return;
      }
      // 别人作品需作者同意
      let req = w.downloadReqs.find(r => r.userId === cur.id);
      if (!req) {
        req = { userId: cur.id, status: 'pending' };
        w.downloadReqs.push(req);
        X.store.saveWork(w);
      ...
      function t_ok() { return X.t('works.download'); }
    },
    ...
    async openRealname(workId) {
      ...
      if (name && idno) {
        const cur = X.auth.currentUser();
        cur.realname = true; cur.realnameInfo = { name, id: idno.slice(0, 4) + '****' };
        X.store.saveUser(cur);
